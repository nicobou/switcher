...
int main() {
  {
    ...
    auto reader = swit->quids<&Container::create>("connection-quid", "reader", nullptr).get();
    assert(reader);

    auto writer = swit->quids<&Container::create>("connection-quid", "writer", nullptr).get();
    assert(writer);

    // get the writer connection information from the Quiddity conspec tree
    std::map<std::string, swid_t> swids;
    {
      auto conspec_tree = writer->conspec<&InfoTree::get_copy>();
      assert(conspec_tree);
    ...
    {
      auto conspec_tree = reader->conspec<&InfoTree::get_copy>();
      assert(conspec_tree);
      assert(!conspec_tree->empty());
    ...
    {  // check compatibility of shmdata before trying to connect
      const auto wtexture_id = writer->claw<&Claw::get_swid>("texture");
      const auto wtypes = writer->claw<&Claw::get_writer_can_do>(wtexture_id);
      assert(!wtypes.empty());
      const auto rtexture_id = reader->claw<&Claw::get_sfid>("texture");
      const auto rtypes = reader->claw<&Claw::get_follower_can_do>(rtexture_id);
      assert(!rtypes.empty());
      // test by ids
      assert(reader->claw<&Claw::can_do_swid>(rtexture_id, writer->get_id(), wtexture_id));
      // test by shmdata types
      bool writer_is_compatible = false;
      for (const auto& it : wtypes) {
        if (reader->claw<&Claw::swid_can_do_shmtype>(rtexture_id, it)) {
...

    {
      ...
      auto rtexture_id = reader->claw<&Claw::get_sfid>("texture");
      assert(Ids::kInvalid != rtexture_id);
      auto res_id = reader->claw<&Claw::connect_quid>(rtexture_id, writer->get_id());
      // since follower texture is not meta, we expect to obtain the same id
      assert(res_id == rtexture_id);
      // and then disconnect
      assert(reader->claw<&Claw::disconnect>(res_id));
